...
async def test_set_merged_after_merge_calls_patch_true_on_success() -> None:
    ...
    assert kwargs["json"] == {"merged_to_target_branch": True}


@pytest.mark.asyncio
async def test_set_merged_after_merge_no_patch_on_failure() -> None:
    http = AsyncMock()
    merge_result = {"merged": False, "error": "conflict"}
    await _set_merged_after_merge(http, "http://x", "task-1", merge_result)
